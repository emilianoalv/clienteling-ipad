...
//   - cl-gabriela via pu-18 (abr-22 PER YSL) → YES (era NO antes del seed expandido)
// Tasa Admin = (3/3) × 100 = 100.0

describe("getRepurchaseRate", () => {
  it("Admin default 6+6: 3 de 3 cohort → 100.0", async () => {
    expect(await getRepurchaseRate(admin, { period: aprilPeriod })).toBe(100);
  });
  ...
  it("Gerente Perisur: cl-elena (sí) + cl-gabriela (sí pu-18) → 100.0", async () => {
    expect(await getRepurchaseRate(gerentePer, { period: aprilPeriod })).toBe(100);
  });
  ...
  it("opts custom (cohort=2, lookback=2): cohort = {rocio, julieta, elena}; repurchased = {elena, rocio via pu-17} → 66.7", async () => {
    expect(
