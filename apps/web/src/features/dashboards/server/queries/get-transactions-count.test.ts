...
describe("getTransactionsCount", () => {
  it("Admin sin filtros cuenta TODAS las compras del período", async () => {
    // Abril 2026: pu-1, pu-3, pu-5, pu-7, pu-9, pu-10 = 6 transacciones
    expect(await getTransactionsCount(admin, { period: aprilPeriod })).toBe(6);
  });
  ...
  it("Supervisor Centro (POL + STF) excluye Perisur", async () => {
    // Abril POL + STF: pu-1, pu-3, pu-9, pu-10 = 4
    expect(
